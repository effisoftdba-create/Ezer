...
function getCompanySvg(name, currentImage) {
  if (typeof currentImage === 'string' && currentImage.trim()) {
    ...
    if (trimmed.includes('<svg') || trimmed.includes('%3Csvg') || trimmed.includes('%3csvg') || trimmed.startsWith('data:image/svg+xml')) {
      let svgStr = trimmed;
      if (svgStr.startsWith('data:image/svg+xml')) {
        ...
        if (commaIdx !== -1) {
          ...
          try {
            svgStr = svgStr.includes(';base64') ? atob(payload) : decodeURIComponent(payload);
          } catch {
            svgStr = '';
          }
        }
    ...
    if (trimmed.startsWith('data:image') || trimmed.startsWith('http') || trimmed.startsWith('/') || trimmed.startsWith('images/')) {
      return null; // Render user custom raster image!
    }
  }
